...
from src import config, preprocessing, features, explainability, history
from src import report, model_comparison
...
def cached_shap(_model, X_bytes, shape, feature_names):
    """
    Calcule les valeurs SHAP, mis en cache pour ne pas recalculer a chaque action.
    X_bytes/shape encodent X pour le cache (les ndarray ne sont pas hashables).
    """
    X = np.frombuffer(X_bytes).reshape(shape)
    return explainability.compute_shap_values(_model, X, feature_names)


def plot_shap_global(shap_result, theme, top=12):
    """Barres horizontales de l'importance globale SHAP des features."""
    imp = explainability.global_feature_importance(shap_result, top=top)
    fig = go.Figure(go.Bar(
        x=imp["importance"][::-1], y=imp["feature"][::-1], orientation="h",
        marker_color=theme["accent"],
    ))
    fig.update_xaxes(title="Importance (|SHAP| moyen)")
    return style_fig(fig, theme, height=380, title="Features les plus influentes")


def plot_shap_bands(shap_result, theme):
    """Importance SHAP regroupee par bande de frequence."""
    ...
    colors = [band_colors.get(b, theme["accent"]) for b in bands["band"]]
    fig = go.Figure(go.Bar(x=bands["band"], y=bands["importance"], marker_color=colors,
                           text=[f"{v:.3f}" for v in bands["importance"]],
                           textposition="auto"))
    fig.update_xaxes(title="Bande")
    fig.update_yaxes(title="Importance cumulée")
    return style_fig(fig, theme, height=300, title="Importance par bande de fréquence")


def plot_shap_channels(shap_result, theme, top=8):
    """Importance SHAP regroupee par canal EEG."""
    chans = explainability.importance_by_channel(shap_result, top=top)
    fig = go.Figure(go.Bar(x=chans["channel"], y=chans["importance"],
                           marker_color=theme["accent2"],
                           text=[f"{v:.3f}" for v in chans["importance"]],
                           textposition="auto"))
    fig.update_xaxes(title="Canal")
    fig.update_yaxes(title="Importance cumulée")
    return style_fig(fig, theme, height=300, title="Importance par canal")
...
# Enregistrement dans l'historique (une seule fois par fichier charge)
sig = f"{uploaded.name}_{result['n_epochs']}_{pred}"
if st.session_state.get("last_saved") != sig:
    try:
        history.add_analysis(uploaded.name, pred, conf, result["n_epochs"],
                             result["probabilities"])
        st.session_state["last_saved"] = sig
    except Exception:
        pass  # l'historique ne doit jamais bloquer l'analyse
...
with r2:
    ...
    topo_band = st.radio("Bande affichée sur la topographie",
                         ["alpha", "beta", "theta"], horizontal=True, index=0)
    st.plotly_chart(plot_topomap(df, theme, band=topo_band),
                    use_container_width=True)
...
# --- Explicabilite IA (SHAP) ---
st.markdown('<div class="ecs-label">4 · Explicabilité IA — pourquoi cette décision ?</div>',
            unsafe_allow_html=True)
st.markdown(
    f'<div style="color:{theme["muted"]}; font-size:13px; margin-bottom:0.6rem;">'
    "SHAP révèle quelles caractéristiques ont le plus pesé dans les décisions du "
    "modèle. C'est l'IA explicable : le modèle ne prédit pas seulement, il se justifie."
    "</div>", unsafe_allow_html=True)

with st.spinner("Calcul des explications SHAP..."):
    # Reconstruire X (features de tous les epochs du fichier charge)
    cleaned_x = preprocessing.clean_signal(df)
    epochs_x = preprocessing.segment_epochs(cleaned_x)
    feat_rows_x = [features.extract_epoch_features(epochs_x[i])
                   for i in range(epochs_x.shape[0])]
    Xdf = pd.DataFrame(feat_rows_x)
    feat_names = list(Xdf.columns)
    Xarr = Xdf.values.astype(np.float64)
    shap_result = cached_shap(model, Xarr.tobytes(), Xarr.shape, feat_names)

xtab1, xtab2, xtab3 = st.tabs(["Bandes de fréquence", "Canaux", "Features détaillées"])
with xtab1:
    st.plotly_chart(plot_shap_bands(shap_result, theme), use_container_width=True)
    bands = explainability.importance_by_band(shap_result)
    dominant = bands.iloc[0]["band"]
    st.markdown(f"La bande **{dominant}** est la plus déterminante pour ce fichier — "
                "cohérent avec la neurophysiologie des états cognitifs.")
with xtab2:
    st.plotly_chart(plot_shap_channels(shap_result, theme), use_container_width=True)
with xtab3:
    st.plotly_chart(plot_shap_global(shap_result, theme, top=12),
                    use_container_width=True)

# --- Historique des analyses (SQLite) ---
st.markdown('<div class="ecs-label">5 · Historique des analyses</div>',
            unsafe_allow_html=True)
try:
    records = history.get_history(limit=20)
    total = history.count_analyses()
except Exception:
    records, total = [], 0

if records:
    st.markdown(
        f'<div style="color:{theme["muted"]}; font-size:13px; margin-bottom:0.5rem;">'
        f"{total} analyse(s) enregistrée(s) — les plus récentes en premier."
        "</div>", unsafe_allow_html=True)
    hist_df = pd.DataFrame([{
        "Date": r["timestamp"],
        "Fichier": r["filename"],
        "État prédit": r["predicted"],
...
# --- Comparateur de modeles ---
st.markdown('<div class="ecs-label">6 · Comparaison de modèles</div>',
            unsafe_allow_html=True)
st.markdown(
    f'<div style="color:{theme["muted"]}; font-size:13px; margin-bottom:0.5rem;">'
    "Entraîne et compare plusieurs algorithmes sur les mêmes données, avec la "
    "même validation par sujet. Nécessite le tableau de features (Sprint 3)."
    "</div>", unsafe_allow_html=True)

if st.button("⚖️ Lancer la comparaison des modèles"):
    feat_csv = config.PROCESSED_DIR / "features.csv"
    if not feat_csv.exists():
        st.warning("Tableau de features introuvable. Lancez d'abord le Sprint 3 "
                   "(notebook 03 ou `python src/features.py`).")
    else:
        with st.spinner("Entraînement et comparaison (peut prendre un moment)..."):
            from src import model as _model_mod
            Xc, yc, gc, _ = _model_mod.load_features()
            try:
                comp = model_comparison.compare_models(Xc, yc, gc, verbose=False)
                st.dataframe(comp, use_container_width=True, hide_index=True)
                best = comp.iloc[0]
                st.success(f"🏆 Meilleur modèle : **{best['Modèle']}** "
                           f"(macro-F1 = {best['Macro-F1']})")
            except Exception as e:
                st.error(f"Comparaison impossible : {e}")

# --- Export du rapport PDF ---
st.markdown('<div class="ecs-label">7 · Exporter le rapport</div>',
            unsafe_allow_html=True)
try:
    pdf_bytes = report.generate_report(df, result, uploaded.name, channel=channel)
    st.download_button(
        "📄 Télécharger le rapport PDF",
        data=pdf_bytes,
        file_name=f"rapport_{uploaded.name.replace('.txt', '')}.pdf",
        mime="application/pdf",
    )
except Exception as e:
    st.caption(f"Rapport indisponible : {e}")
